...
class IncomeExpenseBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, double> incomeMap = {for (var m in last6) m: 0};
    final Map<String, double> expenseMap = {for (var m in last6) m: 0};

    for (var tx in transactions) {
      final key = "${tx.transactionDate.month}/${tx.transactionDate.year}";
      if (last6.contains(key)) {
        if (tx.type == "Income") {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            const Text("Income vs Expense Trend (6 months)",
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            SizedBox(
              height: 250,
              child: BarChart(BarChartData(
                ...
                titlesData: FlTitlesData(
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      showTitles: true,
                      getTitlesWidget: (val, meta) {
                        if (val < 0 || val >= last6.length) return const SizedBox();
                        return Text(last6[val.toInt()],
                            style: const TextStyle(fontSize: 10));
                      },
                    ),
                  ),
                ...
                barGroups: List.generate(last6.length, (i) {
                  final inc = incomeMap[last6[i]] ?? 0;
                  final exp = expenseMap[last6[i]] ?? 0;
                  return BarChartGroupData(x: i, barRods: [
